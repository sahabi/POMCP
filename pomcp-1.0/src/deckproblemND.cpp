...
int FOURBYTHREE::NeighborDegree(int state1, int state2, int n_col = xS) const
{
    int x1 = state1 % n_col;
    int y1 = state1 / n_col;
    int x2 = state2 % n_col;
    ...
    if (state2 >= 0)
    {
        if (state2 < 0)
            ...
            return 1;
        else if ((abs(x1 - x2) == 2 && abs(y1 - y2) <= 2 ) || (abs(x1 - x2) <= 2 && abs(y1 - y2) == 2 ))
            return 2;
...
int FOURBYTHREE::NextState(int from, int dir,int a, int b) const
{
    ...
    int& observation, double& reward) const
{
    ...
    int newstate = NextState(oldstate, action,xS,yS);
...
inline int FOURBYTHREE::MakeObservations(const FOURBYTHREE_STATE& tagstate) const
{
    ...
    if (NeighborDegree(aircraftPos,humanPos) <= 1)
        obs = humanPos;

    else if (NeighborDegree(aircraftPos,humanPos) == 2){

        int obslist [4] = {humanPos,humanPos + 1,humanPos - xS + 1,humanPos - xS};

        if (humanPos == ((yS - 1) * xS)){ //Top Left Corner

            while (NeighborDegree(aircraftPos, obs) > 2){
